...
def cleanDataMain():
    ...
    thread = threading.Thread(target=backgroundTask, args=(x,dict,model_choice,modelType,fname,df_dict,cleanMethodDict,))
    thread.start()

    return "True"

def backgroundTask(x, dict, model_choice, modelType, fname, df_dict, cleanMethodDict):
...
def generalReport():
    ...
    return render_template('generalReport.html',myResult = modelResult, dataClean = dataClean, drop_col = drop_col, modelType = modelType)
...
def detailReport():
    ...
    return render_template('detailReport.html', mydict = mydict, modelName = modelName)
